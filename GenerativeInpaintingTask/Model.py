...
class S1PatchGAN(LightningModule):
    def __init__(
            self,
            image_height: int,
            image_width: int,
        ...
        # networks
        self.generator: Generator_CoarseNet_StageI = \
            Generator_CoarseNet_StageI(in_channels=generator_in_channels)
        self.discriminator = SpectralNormMarkovianDiscriminator(in_channels=discriminator_in_channels)
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            # generate images
            self.refined_result = self(self.incomplete)
            self.complete_result = self.refined_result * self.mask + self.ground_truth * (1. - self.mask)
            ...
            # adversarial loss is binary cross-entropy
            g_l1_loss = self.hparams.l1_loss_alpha * F.l1_loss(self.ground_truth, self.refined_result)
            g_hinge_loss = -torch.mean(self.discriminator(self.complete_result))
            g_loss = g_l1_loss + g_hinge_loss
            self.log("g_loss", g_loss, prog_bar=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # generate images
        refined_result = self(incomplete)
        complete_result = refined_result * mask + ground_truth * (1. - mask)
